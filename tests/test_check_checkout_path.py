...
class TestChekoutPath:
    ...
    def test_check_fill_out_order_form(self, page, driver, name, surname, address, station, phone, date_piker, rental_period, color, comment):
        ...
        with allure.step("Step 8. Выбираем срок аренды из выпадающего списка"):
            with allure.step("Пользователь выбрал срок аренды. Срок отобразился в инпуте"):
                ...
                WebDriverWait(driver, 3).until(EC.presence_of_element_located(page.input_select_rental_period(rental_period)))
    ...
    @pytest.mark.parametrize("name, surname, address, station, phone, date_piker, rental_period, color, comment", person_data)
    def test_check_pop_up_window_successful_order(self, page, driver, methods, name, surname, address, station, phone, date_piker, rental_period, color, comment):
        methods.order_about_person_page(driver, page, name, surname, address, station, phone)
        with allure.step("Тапаем на кнопку «Далее» на первой странице"):
            with allure.step("Пользователь перешел на вторую страницу формы заполнения заказа"):
                driver.find_element(*page.order_next_button).click()
                WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located(page.order_header_about_rent))
        methods.order_about_rental_page(driver, page, date_piker, rental_period, color, comment)
